import type { Metadata } from 'next';
import PageShell from '@/app/components/PageShell';
import PageHero from '@/app/components/PageHero';
import { SITE } from '@/lib/site';
import { mergeShowcaseProjects } from '@/lib/projets';
import { fetchShowcaseProjects } from '@/lib/api';

export const revalidate = 300;

export const metadata: Metadata = {
  title: "Projets open-source · Y'TILIKAN",
  description:
    "AfriBench, AfroLang-Library, AfroTech-Pulse, Dira Browser : l'écosystème open-source Y'TILIKAN pour une IA africaine.",
  alternates: { canonical: '/projets' },
};

export default async function ProjetsPage() {
  const projets = mergeShowcaseProjects(await fetchShowcaseProjects());
  return (
    <PageShell active="projets">
      <PageHero
        eyebrow="04 / Initiatives & projets open-source"
        title="Notre écosystème open-source."
        lead="Tous nos projets sont ouverts, documentés et prêts aux contributions. De l'évaluation des LLMs aux datasets de langues africaines : l'infrastructure d'une IA africaine se construit ici."
      />

      <section className="page-section">
        <div className="wrap">
          <div className="proj-detail-list">
            {projets.map((p) => (
              <article className="proj-detail reveal" id={p.slug} key={p.slug}>
                <div className="proj-detail__top">
                  <span className="pj__tag">{p.tag}</span>
                  <span
                    className={`pj__status ${
                      p.status.toLowerCase().includes('dev') ? 'pj__status--wip' : 'pj__status--live'
                    }`}
                  >
                    {p.status}
                  </span>
                </div>
                <h2 className="proj-detail__name">{p.name}</h2>
                <p className="proj-detail__detail">{p.detail}</p>
                <p className="proj-detail__desc">{p.desc}</p>
                {p.highlights.length > 0 && (
                  <ul className="proj-detail__highlights">
                    {p.highlights.map((h) => (
                      <li key={h}>{h}</li>
                    ))}
                  </ul>
                )}
                {p.stack.length > 0 && (
                  <ul className="pj__stack" aria-label="Technologies">
                    {p.stack.map((s) => (
                      <li key={s}>{s}</li>
                    ))}
                  </ul>
                )}
                {p.authors ? <p className="pj__authors">{p.authors}</p> : null}
                <div className="pj__links">
                  {p.github && p.github !== '#' && (
                    <a
                      href={p.github}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="pj__link pj__link--gh"
                    >
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                      <path d="M12 0C5.37 0 0 5.37 0 12c0 5.3 3.438 9.8 8.205 11.387.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.73.083-.73 1.205.085 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.418-1.305.762-1.604-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 21.795 24 17.295 24 12c0-6.63-5.37-12-12-12z" />
                    </svg>
                    GitHub
                  </a>
                  )}
                  {p.site && (
                    <a
                      href={p.site}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="pj__link pj__link--web"
                    >
                      Voir le site
                    </a>
                  )}
                </div>
              </article>
            ))}
          </div>
        </div>
      </section>

      <section className="page-cta-band">
        <div className="wrap page-cta-band__inner reveal">
          <h2 className="page-cta-band__title">Contribuer dès maintenant</h2>
          <p className="page-cta-band__lead">
            Issues, pull requests, datasets : tout se passe sur l&apos;organisation GitHub YTILIKAN.
          </p>
          <div className="page-cta-band__actions">
            <a href={SITE.github} target="_blank" rel="noopener noreferrer" className="btn btn--gold">
              Ouvrir GitHub
            </a>
            <a href="https://community.ytilikan.org/connexion" className="btn btn--ghost page-cta-band__ghost">
              Rejoindre l&apos;espace membre
            </a>
          </div>
        </div>
      </section>
    </PageShell>
  );
}
