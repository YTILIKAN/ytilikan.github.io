'use client';

import { mergeShowcaseProjects } from '@/lib/projets';
import type { ShowcaseProject } from '@/lib/api';

interface Props {
  apiProjects?: ShowcaseProject[];
}

export default function Projets({ apiProjects }: Props) {
  const source = mergeShowcaseProjects(apiProjects ?? []);

  return (
    <section className="section" id="projets">
      <div className="wrap">
        <div className="section-head reveal">
          <span className="eyebrow">04 / Initiatives &amp; projets open-source</span>
          <h2 className="sec-title">Nos projets open-source.</h2>
          <p className="lead">
            Nos projets sont ouverts et documentes. Evaluation de LLMs, datasets de langues
            africaines, outils de veille.
          </p>
        </div>

        <div className="pj-list reveal">
          {source.map((p) => {
            const wip = p.status.toLowerCase().includes('dev');
            return (
              <article className="pj" key={p.slug}>
                <div className="pj__top">
                  <span className="pj__tag">{p.tag}</span>
                  <span className={`pj__status ${wip ? 'pj__status--wip' : 'pj__status--live'}`}>
                    {p.status}
                  </span>
                </div>
                <h3 className="pj__name">{p.name}</h3>
                <p className="pj__desc">{p.desc}</p>
                {p.highlights.length > 0 && (
                  <ul className="pj__highlights">
                    {p.highlights.map((h) => (
                      <li key={h}>{h}</li>
                    ))}
                  </ul>
                )}
                {p.stack.length > 0 && (
                  <ul className="pj__stack" aria-label="Technologies">
                    {p.stack.map((s) => (
                      <li key={s}>{s}</li>
                    ))}
                  </ul>
                )}
                <div className="pj__foot">
                  <p className="pj__authors">{p.authors}</p>
                  <div className="pj__links">
                    {p.github !== '#' && (
                      <a
                        href={p.github}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="pj__link pj__link--gh"
                      >
                        <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                          <path d="M12 0C5.37 0 0 5.37 0 12c0 5.3 3.438 9.8 8.205 11.387.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.73.083-.73 1.205.085 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.418-1.305.762-1.604-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 21.795 24 17.295 24 12c0-6.63-5.37-12-12-12z" />
                        </svg>
                        GitHub
                      </a>
                    )}
                    {p.site && (
                      <a
                        href={p.site}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="pj__link pj__link--web"
                      >
                        Site
                      </a>
                    )}
                    <a href={`/projets#${p.slug}`} className="pj__link pj__link--more">
                      Details
                    </a>
                  </div>
                </div>
              </article>
            );
          })}
        </div>

        <div className="pj-cta reveal">
          <a href="/projets" className="section-more">
            Fiches projets completes
          </a>
        </div>
      </div>
    </section>
  );
}